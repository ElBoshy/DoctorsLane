...
@section('content')
        ...
        <div class="form-group">
            ...
            <select name="speciality_id" class="form-control" >
                ...
                @foreach ($specialities as $speciality)
                    <option @if($speciality->id==$currentUser->speciality_id) selected @endif value="{{$speciality->id}}">{{$speciality->name}}</option>
                @endforeach
            </select>
        </div>
        @endif
    ...
    @if($currentUser->role_id == 3)
    <a href="{{route('index')}}"><button class="btn btn-info">Next To Select Your Pain</button></a>
    @endif
@stop
